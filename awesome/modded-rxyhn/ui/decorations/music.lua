...
--- Volume slider
local vol_button = widgets.button.text.normal({
...
local vol_tooltip = wibox.widget({
	visible = false,
	top_only = true,
...
local tooltip_text = wibox.widget({
...
awesome.connect_signal("signal::volume", function(value, muted)
	...
	if muted == 1 or value == 0 then
		...
		vol_button:set_text("")
		fill_color = beautiful.accent
	end

	vol_slider.slider.value = vol_value
	vol_slider.slider.color = fill_color
	tooltip_text.markup = helpers.ui.colorize_text(vol_value .. "%", beautiful.accent)
end)
